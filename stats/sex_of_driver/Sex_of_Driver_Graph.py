...
# plt.hist(merge_df["Sex_of_Driver"], bins=bins, label=["male", "female", "unknow"])

# pd.DataFrame({'lab':['A', 'B', 'C'], 'val':[10, 30, 20]})
print("output")
print("="*50)
output = merge_df["Sex_of_Driver"].value_counts()
print(output)

bars = pd.DataFrame({'label':["male", "female", "unknow"], 'frequency':output.tolist()[0:3]})

bars.plot.bar(x='label', y='frequency', rot=0, ylim=(0, 3000000))
